...
class LinfPGDAttack:
  ...
  def perturb(self, x_nat, x_adv, y, sess):
    ...
    for epoch in range(self.k):
      ...
      x_0 = x_adv + self.a * np.sign(grad)
                                    
      grad_0 = sess.run(self.grad, feed_dict={self.model.x_input: x_0,
                                           self.model.y_input: y})
                                         
      grad_all = (p * grad + (1-p) * grad_0)

      x_adv += self.a * np.sign(grad_all)
...
if __name__ == '__main__':
  ...
  with tf.Session() as sess:
    ...
    for restart in range(50):
      ...
      adv_acc = sess.run(attack.model.accuracy, feed_dict={attack.model.x_input: x_adv_final,
                                            attack.model.y_input: mnist.test.labels})
    ...
    path = "/home/wanchen/adversarial_training/mnist_challenge-master/outputs/pgd_ie_p/p01"
    print(accs)
